...
ai.load('agnellopio.brn') if File.file?('agnellopio.brn')
...
def save_private_ryan(real_ai)
  if $save_counter > 20
    begin
      File.delete('agnellopio.bak')
    rescue
      puts 'Cannot delete backup.'
    end

    begin
      File.rename('agnellopio.brn', 'agnellopio.bak')
    rescue
      puts 'Cannot rename actual brain.'
    end
...
Telegram::Bot::Client.run(token) do |bot|
  bot.listen do |message|
    ...
    when Telegram::Bot::Types::Message
      ...
      # Triggers
      if message.text =~ /@AgnelloPio_bot/i
        reply = ai.reply(text)
        bot.api.send_message(chat_id: message.chat.id, text: reply, reply_to_message_id: message.message_id)
        save_private_ryan(ai)
        next
